...
try {
    ...
    if (!$gw) {
        $pdo->prepare("INSERT INTO gateways (name, active) VALUES ('Oasyfy', 1)")->execute();
        $stmt->execute();
        $gw = $stmt->fetch();
    }
} catch (Exception $e) {
...
if (!$gw) {
    echo json_encode(['success' => false, 'error' => 'Configuração Oasyfy não encontrada.']);
    exit;
}
...
if (empty($config['public_key']) || empty($config['secret_key'])) {
    echo json_encode(['success' => false, 'error' => 'Chaves da Oasyfy não configuradas. Acesse Admin > Gateways.']);
    exit;
}
...
// Phone formatting for Oasyfy (XX) XXXXX-XXXX
...
file_put_contents('oasyfy_debug.log', json_encode($debug_data, JSON_PRETTY_PRINT) . "\n---\n", FILE_APPEND);
